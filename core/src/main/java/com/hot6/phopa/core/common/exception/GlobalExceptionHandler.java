...
import com.hot6.phopa.core.common.model.entity.ResponseCommonEntity;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    @ExceptionHandler(value = AdminErrorException.class)
    public ResponseEntity<Object> handleAdminErrorException(WebRequest request, AdminErrorException ex) {
        return handleAdminRuntimeException(ex, request);
    }

    @ExceptionHandler(value = SilentAdminErrorException.class)
    public ResponseEntity<Object> handleSilentAdminErrorException(WebRequest request, AdminErrorException ex) {
        return handleAdminRuntimeException(ex, request);
    }

    private ResponseEntity<Object> handleAdminRuntimeException(AdminErrorException ex, WebRequest request) {
        AdminErrorType adminErrorType = ex.getResponseStatusType();

        String errorMessage;
        if (StringUtils.isNotEmpty(ex.getCustomMessage())) {
            errorMessage = ex.getCustomMessage();
        } else {
            errorMessage = ex.getMessage();
        }

        ResponseCommonEntity<Object> responseCommonEntity = ResponseCommonEntity.builder()
            .code(adminErrorType.getCode())
            .message(errorMessage)
            .build();

        return handleExceptionInternal(ex, responseCommonEntity, new HttpHeaders(), adminErrorType.getStatus(), request);
    }
    ...
    protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body,
        ...
        } else if (ex instanceof SilentAdminErrorException) {
            AdminErrorException aEE = (AdminErrorException) ex;
            log.error("SilentAdminErrorException.{} [code = {}]", aEE.getResponseStatusType().name(), aEE.getResponseStatusType().getCode());
        } else {
            log.error("", ex);
        }
